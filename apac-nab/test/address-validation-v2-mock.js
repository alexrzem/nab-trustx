...
function validateAddress(sessionData, address, successCallback, errorCallback) {
    ...
    xhr.open('POST', sessionData['nabAddressServiceBaseUrl'] + sessionData['nabAddressValidateUrl'], true);
...
function getAddress(sessionData, addressId, successCallback, errorCallback) {
    let token = sessionData?.idv['addressToken'] ?? sessionData?.idv?.config['openAPIToken']
    const xhr = new XMLHttpRequest();
    xhr.open('GET', sessionData['nabAddressServiceBaseUrl'] + sessionData['nabAddressGetUrl'] + "/" + addressId, true);
    // xhr.setRequestHeader('x-nab-accesstoken', token ?? ""); // NB-760
    xhr.setRequestHeader('Authorization', 'Bearer ' + token ?? "");
    xhr.setRequestHeader('x-correlationid', uuidV4()); // NB-759
    xhr.setRequestHeader("Accept", "application/json");
    ...
    const address = {
        "fullAddress": "3 George St, Oaks Estate, ACT",
        "unitType": "",
        "unitNumber": "",
        "floorLevel": "",
        ...
        "streetType": "St",
        "streetTypeSuffix": "",
        "suburbPlaceLocality": "Oaks Estate",
        "stateTerritory": "ACT",
        "country": "AU",
        "postCode": "2620",
        "postalDeliveryType": "",
...
function searchAddress(sessionData, addressString, countryCode, successCallback, errorCallback) {
    let token = sessionData?.idv['addressToken'] ?? sessionData?.idv?.config['openAPIToken']
    const xhr = new XMLHttpRequest();
    xhr.open('POST', sessionData['nabAddressServiceBaseUrl'] + sessionData['nabAddressSearchUrl'], true);
    // xhr.setRequestHeader('x-nab-accesstoken', token ?? ""); // NB-760
    xhr.setRequestHeader('Authorization', 'Bearer ' + token ?? "");
    xhr.setRequestHeader('x-correlationid', uuidV4()); // NB-759
    xhr.setRequestHeader("Accept", "application/json");
...
function populateAddressFields(address) {
    ...
    // NB-763
    const input = document.querySelector('#documentAddressHeader');
...
function bindAddressSearchInput() {
    ...
    // addressSuggestions.anchorCorner = 'end-end'
    // addressSuggestions.menuCorner = 'end-end'
...
function getDataZooAddress() {
    let dataZooFormat = sessionData.idv.dataZooAddressFormat
    if (!dataZooFormat) {
       dataZooFormat = '{documentAddressLevelNumber} {documentAddressBuildingPropertyName} {documentAddressUnitNumber} {documentAddressStreetNumber} {documentAddressStreetName} {documentAddressStreetType}, {documentAddressSuburb} {documentAddressState} {documentAddressPostalCode} {documentAddressCountry}'
    }
